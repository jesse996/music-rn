import type { IInfo, ITrack } from '@/types'
...
export async function get_playlist_filters(): Promise<IPlaylistFilters> {
  const target_url = `https://c.y.qq.com/splcloud/fcgi-bin/fcg_get_diss_tag_conf.fcg?picmid=1&rnd=${Math.random()}&g_tk=732560869&loginUin=0&hostUin=0&format=json&inCharset=utf8&outCharset=utf-8&notice=0&platform=yqq.json&needNewCode=0`
  ...
  // console.log('get_playlist_filters:', json.data.categories)

  const all = [] as IAll[]
  for (const cate of json.data.categories) {
    const result = {
...
export async function qq_get_playlist(playlistId: string): Promise<IPlayList> {
  const list_id = playlistId.split('_').pop()
  console.log('qq_get_playlist:', list_id)
...
function getParameterByName(name: string, url: string) {
  ...
  console.log('qq_get_playlist:', res)
...
export async function show_playlist(
  ...
  const target_url = `https://c.y.qq.com/splcloud/fcgi-bin/fcg_get_diss_by_tag.fcg?picmid=1&rnd=${Math.random()}&g_tk=732560869&loginUin=0&hostUin=0&format=json&inCharset=utf8&outCharset=utf-8&notice=0&platform=yqq.json&needNewCode=0&categoryId=${filterId}&sortId=5&sin=${offset}&ein=${
    29 + offset
  }`
  console.log('playList url:', target_url)

  // let res = await fetch("http://localhost:5105", {
  //     method:'POST',
  //     headers: {
  //         'Accept': 'application/json',
  //         'Content-Type': 'application/json'
  //     },
  //     body:JSON.stringify({to:target_url}),
  //     // headers: {
  //     //     Referer: 'https://y.qq.com/',
  //     // },
  // })
...
export async function bootstrap_track(trackId: string) {
  ...
  const file =
    songmidList.length === 1 && `${fileInfo.s}${songId}${songId}${fileInfo.e}`
  ...
  const res = await fetch(target_url + new URLSearchParams(params), {
    headers: {
      Referer: 'https://y.qq.com/',
      // host: 'u.y.qq.com',
    },
  ...
  const data = await res.json()
  console.log(data.req_0.data.midurlinfo[0])
  ...
  sound.url = data.req_0.data.sip[0] + purl
...
//歌词
export async function lyric(track_id: string): Promise<ILyric> {
  track_id = track_id.slice('qqtrack_'.length)
  const target_url = `https://i.y.qq.com/lyric/fcgi-bin/fcg_query_lyric_new.fcg?songmid=${track_id}&g_tk=5381&format=json&inCharset=utf8&outCharset=utf-8&nobase64=1`
  const res = await fetch(target_url, {
    headers: {
      Referer: 'https://y.qq.com/',
    },
